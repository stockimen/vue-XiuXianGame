...
<script setup>
  ...
  // 自动收服流程：先尝试收服，失败再战斗
  const autoCaptureFlow = () => {
    if (isEnd.value) return
    harvestPet(monster.value)
    // harvestPet 是同步的：成功则调了 stopFight()（isEnd=true），失败则 isEnd 不变
    setTimeout(() => {
      if (!isEnd.value) {
        texts.value.push('收服失败，自动发起战斗')
        startFight()
      }
    }, 200)
  }
  ...
  const stopFight = () => {
    ...
    // 自动模式：战斗结束后自动处理
    if (isAutoMode.value && (store.autoExploring || store.autoFight)) {
      setTimeout(() => {
        if (player.value.health <= 0) {
          if (store.autoExploring) store.autoExploring = false
          goHome()
        } else {
          // 战斗后恢复10%最大血量
          const heal = Math.floor(player.value.maxHealth * 0.1)
          player.value.health = Math.min(player.value.health + heal, player.value.maxHealth)
          router.push('/map')
        }
      }, 1500)
    }
  ...
  onMounted(() => {
    ...
    // 自动模式：自动发起战斗或收服
    const shouldAutoFight = store.autoExploring || store.autoFight
    if (shouldAutoFight && monster.value.name) {
      isAutoMode.value = true
      setTimeout(() => {
        if (!isEnd.value) {
          if (store.autoCapture && player.value.backpackCapacity > player.value.pets.length) {
            autoCaptureFlow()
          } else {
            startFight()
          }
