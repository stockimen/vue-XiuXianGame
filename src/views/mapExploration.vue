<template>
  <div>
    ...
    <div class="auto-explore-bar" v-if="!fishingShow && !npcShow">
      <div class="auto-explore-main">
        <el-button
          :type="store.autoExploring ? 'danger' : 'success'"
          @click="toggleAutoExplore"
          :disabled="player.health <= 0"
        >
          {{ store.autoExploring ? '停止自动探索' : '开始自动探索' }}
        </el-button>
      </div>
...
<script setup>
  ...
  // 自动探索相关
  const autoMoveTimer = ref(null)
  const lastDirection = ref(null)
  const directionHistory = ref([])
  // 自动寻路相关
  ...
  // A* 寻路算法 — 返回从 startCoord 到 endCoord 的路径
  const findPath = (startCoord, endCoord) => {
    ...
    gScore.set(startCoord.toString(), 0)
    fScore.set(startCoord.toString(), heuristic(startCoord, endCoord))
    openSet.add(startCoord, fScore.get(startCoord.toString()))
    while (!openSet.isEmpty()) {
      ...
      for (const [dy, dx] of directions) {
        ...
        if (
          newY >= 0 && newY < gridSize.value &&
          newX >= 0 && newX < gridSize.value &&
          ...
          if (!gScore.has(neighbor.toString()) || tentativeGScore < gScore.get(neighbor.toString())) {
            ...
            const fScoreValue = tentativeGScore + heuristic(neighbor, endCoord)
  ...
  const isPathAvailable = () => {
    return findPath([playerY.value, playerX.value], [49, 49]) !== null
  }

  // 自动寻路 — 获取下一步方向
  const getNextPathDirection = () => {
    if (!currentPath.value.length || pathStepIndex.value >= currentPath.value.length) return null
    const [nextY, nextX] = currentPath.value[pathStepIndex.value]
    // 验证下一步仍可通行
    if (grid.value[nextY * gridSize.value + nextX].type !== 'empty') {
      currentPath.value = []
      pathStepIndex.value = 0
      return null
    }
    const dy = nextY - playerY.value
    const dx = nextX - playerX.value
  ...
  const gridInfo = (index, item) => {
    ...
    if (store.autoPathfind) {
      startAutoPathfind(y, x)
      return
    }
    // 自动寻路关闭：原有弹窗逻辑（取消传送 / 立即传送）
  ...
  const chooseDirection = () => {
    ...
    const validDirs = allDirs.filter(d => {
      if (checkDirection(d)) return false
      if (d === 'up' && playerY.value === 0) return false
      if (d === 'down' && playerY.value === gridSize.value - 1) return false
      if (d === 'left' && playerX.value === 0) return false
      if (d === 'right' && playerX.value === gridSize.value - 1) return false
      return true
    })
    if (validDirs.length === 0) return null

    // 卡住检测：最近6步中有3对来回则强制换方向
    if (directionHistory.value.length >= 6) {
      const recent = directionHistory.value.slice(-6)
      let pairs = 0
      for (let i = 0; i < recent.length - 1; i += 2) {
        if (recent[i] === oppositeMap[recent[i + 1]]) pairs++
      }
      if (pairs >= 2) {
        const nonRecent = validDirs.filter(d => !directionHistory.value.slice(-3).includes(d))
        if (nonRecent.length > 0) return nonRecent[Math.floor(Math.random() * nonRecent.length)]
      }
    }

    // 优先不回头
    const preferred = validDirs.filter(d => d !== oppositeMap[lastDirection.value])
    const pool = preferred.length > 0 ? preferred : validDirs

    // 当前方向加权（权重3:1）
    if (lastDirection.value && pool.includes(lastDirection.value)) {
      const weights = pool.map(d => d === lastDirection.value ? 3 : 1)
  ...
  const resumeAutoMove = () => {
    ...
    autoMoveTimer.value = setInterval(() => {
      ...
      if (store.autoPathfind) {
        ...
        if (!dir) {
          // 到达目标位置，停止自动探索
          gameNotifys({ title: '自动寻路', message: '已到达目标位置', position: 'top-left' })
        ...
        if (!dir) {
          gameNotifys({ title: '自动探索', message: '无路可走，已停止', position: 'top-left' })
          stopAutoExplore(false)
          return
        }
        lastDirection.value = dir
        directionHistory.value.push(dir)
        if (directionHistory.value.length > 10) directionHistory.value.shift()
        move(dir)
      }
    }, 500)
  }
  ...
  const toggleAutoExplore = () => {
    if (store.autoExploring) {
      stopAutoExplore(false)
    } else {
      store.autoExploring = true
      resumeAutoMove()
    }
  }

  // 点击地图空地自动前往目标
  const startAutoPathfind = (targetY, targetX) => {
    const path = findPath([playerY.value, playerX.value], [targetY, targetX])
    if (!path) {
      gameNotifys({ title: '自动寻路', message: '无法到达目标位置', position: 'top-left' })
      return
    }
    store.pathfindTarget = [targetY, targetX]
    store.autoExploring = true
    store.autoPathfind = true
    currentPath.value = path
    pathStepIndex.value = 0
    resumeAutoMove()
    gameNotifys({ title: '自动寻路', message: `正在前往 (${targetX}, ${targetY})`, position: 'top-left' })
  }

  // 自动选项互斥逻辑
  const onAutoCaptureChange = (val) => {
    if (val) store.autoFight = true
  }
  const onAutoFightChange = (val) => {
    if (!val) store.autoCapture = false
  }
  ...
  const getRandomFish = () => {
    ...
    const weight = parseFloat((Math.pow(Math.random(), 0.5) * 30 + 1).toFixed(2))
    const fishList = [
  ...
  onMounted(() => {
    ...
    // 恢复自动探索（从战斗页返回时）
    if (store.autoExploring) {
      if (store.autoPathfind && store.pathfindTarget) {
        const path = findPath([playerY.value, playerX.value], store.pathfindTarget)
        if (path) { currentPath.value = path; pathStepIndex.value = 0 }
        else {
          // 路径不通，停止
          gameNotifys({ title: '自动寻路', message: '路径已被阻断，已停止', position: 'top-left' })
          stopAutoExplore(false)
          return
        }
      }
